...
def test_contact_info_edit_vs_db(app, db):
    if app.contact.count() == 0:
        app.contact.contact_create(
            Contact(firstname="Сергей", middlename="Сергеевич", lastname="Сергеев", nickname="Серега",
                    address="г. Казань",
    ...
    contact_from_db = db.get_contact_list()
    assert sorted(contact_from_edit_page, key=Contact.id_or_max) == sorted(contact_from_db, key=Contact.id_or_max)
...
#def test_contact_info_home_vs_edit(app, db):
    ...
    #db.get_contact_list() = app.contact.get_contact_info_from_edit_page()
